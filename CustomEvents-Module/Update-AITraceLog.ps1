...
function Update-AITraceLog() {
    ...
    process {
        ...
        if ($customObject) {
            ...
            # Set Custom Work Object
            $customTempObject = $params.EventDictionary.custom_object | ConvertFrom-Json

            #
            # Prune Excessive Objects from the list

            #
            # Enter the loop
            While ($params.EventDictionary.custom_object.Length -gt $EventCharLimit) {

                # Fetch the Object that has the Highest Length
                $HighestObjectLength = $customTempObject | Get-Member -MemberType Property, NoteProperty | Select-Object -Property Name, @{Name="ObjectLength";Expression={$customTempObject."$($_.Name)" | ConvertTo-Json -Compress -Depth 1 | Select-Object -ExpandProperty Length }} | Sort-Object -Property ObjectLength -Descending | Select-Object -First 1
                # Write Warning
                Write-Warning -Message "Length is too High. Pruning: $($HighestObjectLength.Name)"
                # Prune from the Object
                $customTempObject.PSObject.Properties.Remove($HighestObjectLength.Name)
                # Update the Object
                $params.EventDictionary.custom_object = $customTempObject | ConvertTo-Json -Depth 2 -Compress

            }

            #
            # Prune Excessive Objects with Properties from the List

            $ObjectPropertyCount = $customTempObject | `
                             Get-Member -MemberType Property, NoteProperty | `
                             Select-Object -Property @{
                                 Name="PropertyCount";
                                 Expression={
                                     $customTempObject."$($_.Name)" | `
                                     Get-Member -MemberType NoteProperty, Property | `
                                     Measure-Object | `
                                     Select-Object -ExpandProperty Count }
                            } | Measure-Object -Property PropertyCount -Sum

            While ($ObjectPropertyCount.Sum -gt $PropertyLimit) {

                # Fetch the Property that has the Highlest Length
                $HighestObjectPropertyCount = $customTempObject | Get-Member -MemberType Property, NoteProperty | Select-Object -Property Name, @{Name="PropertyCount";Expression={$customTempObject."$($_.Name)" | Get-Member -MemberType NoteProperty, Property | Measure-Object | Select-Object -ExpandProperty Count }} | Sort-Object -Property PropertyCount -Descending | Select-Object -First 1

                # Write Warning
                Write-Warning -Message "Property Limit is too High. Pruning: $($HighestObjectPropertyCount.Name)"

                # Prune The Object
                $customTempObject.PSObject.Properties.Remove($HighestObjectPropertyCount.Name)

                # Update the Object
                $params.EventDictionary.custom_object = $customTempObject | ConvertTo-Json -Depth 2 -Compress

                # Recaculate the Object Property Count
                $ObjectPropertyCount = $customTempObject | `
                                    Get-Member -MemberType Property, NoteProperty | `
                                    Select-Object -Property @{
                                        Name="PropertyCount";
